...
// handleGetMyRoleRequest handles getMyRole operation.
//
// Get my role in chat.
//
// GET /roles/my
func (s *Server) handleGetMyRoleRequest(args [0]string, argsEscaped bool, w http.ResponseWriter, r *http.Request) {
	statusWriter := &codeRecorder{ResponseWriter: w}
	w = statusWriter
	ctx := r.Context()
	...
	)
	{
		...
		var satisfied bitset
		{
			sctx, ok, err := s.securityBearerAuth(ctx, GetMyRoleOperation, r)
			if err != nil {
				err = &ogenerrors.SecurityError{
					OperationContext: opErrContext,
					Security:         "BearerAuth",
	...
	params, err := decodeGetMyRoleParams(args, argsEscaped, r)
	if err != nil {
		err = &ogenerrors.DecodeParamsError{
			OperationContext: opErrContext,
			Err:              err,
	...
	var response GetMyRoleRes
	if m := s.cfg.Middleware; m != nil {
		mreq := middleware.Request{
			Context:          ctx,
			OperationName:    GetMyRoleOperation,
			OperationSummary: "Get my role in chat",
		...
		type (
			Request  = struct{}
			Params   = GetMyRoleParams
			Response = GetMyRoleRes
		)
		response, err = middleware.HookMiddleware[
			Request,
			Params,
			...
			m,
			mreq,
			unpackGetMyRoleParams,
			func(ctx context.Context, request Request, params Params) (response Response, err error) {
				response, err = s.h.GetMyRole(ctx, params)
				return response, err
			},
		)
	} else {
	...
	if err := encodeGetMyRoleResponse(response, w); err != nil {
		defer recordError("EncodeResponse", err)
		if !errors.Is(err, ht.ErrInternalServerErrorResponse) {
			s.cfg.ErrorHandler(ctx, w, r, err)
		}
